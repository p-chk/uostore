...
from tensorflow.keras.layers import Conv2D, MaxPooling2D, Flatten, Dense, Reshape, InputLayer, LSTM
...
def get_model(optimizer='adam'):
    ...
    return model
rnn_model = KerasClassifier(build_fn=get_model)

# COMMAND ----------

rnn_pipeline = ImbPipeline(steps=[
...
grid_search = GridSearchCV(estimator=rnn_pipeline, param_grid=param_grid, cv=cv, scoring='accuracy', n_jobs=-1)
...
with mlflow.start_run(run_name="exp-02-rnn"):
    ...
    # Log model
    mlflow.sklearn.log_model(best_model, "recurrent_neural_network")
