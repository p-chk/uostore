...
def get_model(optimizer='adam'):
    ...
    return model
cnn_model = KerasClassifier(build_fn=get_model)

# COMMAND ----------

cnn_pipeline = ImbPipeline(steps=[
...
grid_search = GridSearchCV(estimator=cnn_pipeline, param_grid=param_grid, cv=cv, scoring='accuracy', n_jobs=-1)
