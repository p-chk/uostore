...
pd.DataFrame(X_train_transformed_loaded)

# COMMAND ----------

selector = SelectKBest(score_func=f_classif, k=5)
X_new = selector.fit_transform(X_train_transformed_loaded, y_train)

# Get the selected feature names
selected_features = pd.DataFrame(X_train_transformed_loaded).columns[selector.get_support(indices=True)]
print(f"Selected features: {selected_features}")
X_train_transformed_loaded = X_new

# COMMAND ----------

X_val_transformed_loaded = selector.transform(X_val_transformed_loaded)
...
with mlflow.start_run(run_name="exp-01-05-feature"):
    ...
    # Log model
    mlflow.sklearn.log_model(lr_pipeline, "logistics_regressions")
    
    # Log parameters, metrics, or any other information if needed
    mlflow.log_param("test_size", 0.2)
    mlflow.log_param("n", 5)
